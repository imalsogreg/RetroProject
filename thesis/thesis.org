...
*** Model 3: Multiple theta sources

Theta is not a single unified phenomenon, but the combination of a number of factors [@buzsaki2002theta]: intrinsic rhythmic tendendencies [???] and interactions among currents [???]. There is not a one-to-one mapping between underlying factors (the the rhythmic spiking of inputs, or the rhythmic modulation of particular currents), and physiological endpoints (such as the spiking of pyramidal cells, the spiking of interneurons, or the oscillation in the local field potential. One example of this dissociation is the GABAergic current at CA1 pyramidal cells. This current comes through an interneuron-to-interneuron-to-pyramidal cell pathway beginning at the medial septum [???] (a highly theta-rhythmic structure [???]) and ending at the pyramidal cell soma [???]. The resulting inhibitory current causes very little deviation on the membrane voltage because the reversal potential of chloride is near the resting membrate potential. But it has a strong effect on spiking, by acting as a somatic shunting current counteracting the excitatory impulses that otherwise drive the cell to spike.

The distinction between different oscillatory factors could explain the apparent paradox of synchronized theta sequences across regions with desynchronized theta oscillations as illustrated in Figure ???. In the rate-to-phase model of Mehta and Wilson [@mehta2002role], what exactly is the inhibitory oscilation? It could be directly related to excitatory currents, or inhibitory currents, the model doesn't specify and there is not experimetal evidence to distinguish the two. There may be phase offsets among excitatory currents but phase synchrony among inhibitory currents, as the latter arise primarily from a single nucleus and the latter arise from interactions between several anatomically large, excitatory inputs. If it is the inhibitory shunting current that determines the precise spike times of place cells, and the inhibitory source of theta is phase synchronized, we would expect theta sequences to be anatomically synchronized.

In this scenario, how could the inhibitory currents be in phase with excitatory currents in the medial hippocampus and nearly 180 degrees out of phase with excitatory currents in the most lateral extent of hippocampus, and how could there be intermediate phase relationships inbetween? We propose that inhibitory and excitatory components are independent, with inhibitory currents arising directly from the synchronized septal pacemaker, and excitatory currents coming from various combinations of glutamatergic afferents that each have their own particular oscillatory features. Taking CA3 and entorhinal cortex layer III again as examples of excitatory inputs, the anatomical distribution of their synapses on the apical dendrites of CA1 pyramidal cells induces dipoles with large charge separation and greater visibility to electrodes than the relatively much smaller dipoles generated by GABAergic synapses on the soma. The excitatory currents dominate the tetrode's view of the underlying theta.

A simple model with CA3 and entorhinal cortex preferring to spike at different absolute theta phases can produce the traveling wave pattern observed in the local field potential, if CA3 and entorhinal cortex have gradients of excitatory influence on CA1 with the gradients going in opposing directions. At any point within CA1, the phase of LFP theta is due to the weighted summed dipoles of caused by the CA3 and EC inputs. As the relative influence shifts anatomically from CA3 to EC, the phase of CA1 theta shifts from that of CA3 to that of EC. The dominance of excitatory currents over LFP theta obscures the presence of the inhibitory theta oscillation and its non-traveling nature.

This model retains the biophysical relationship between theta rhythms and theta sequences in CA1 by attributing the inhibitory oscillation of Mehta and Wilson's [@mehta2002role] excitation-to-phase model to anatomically synchronized inhibitory shunting currents, and the LFP currents to dipoles generated by independent excitatory afferents. It also suggests that the inhibitory basket cells contributing the inhibitory theta currents should be synchronized along the medial-lateral axis. This prediction could be tested, so long as other interneurons in the viscinity, which may difficult to distinguish from basket cells, do not obscure the interneuron timing preference estimates.
