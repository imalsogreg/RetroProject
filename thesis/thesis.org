...
*** Theta sequences
Place cells spike at precise phases of theta that depend on where a rat is within the place field. The spikes at the beginning of the place cell occur at early phases and spike near the end of the field occur at later phases [@o1993phase]. This lead to the prediction that a collection of cells with partlialy overlapping fields will fire in a strict order according to the relative positions of the place fields [@skaggs1996theta; @dragoi2006theta]. Later these sequences were observed directly in large groups of simultaneously recorded neurons [Foster TODO sequences, TODO Patel? in Reddish lab; TODO davidson & kloosterman]. Interestingly, the tight temporal alignment of place cells in theta sequences is greater than the precision with which individual cells align their spikes to the theta oscillation [TODO foster sequences]. 

*** Tension between hypothesized roles in gating communication channels and encoding
These two roles for theta oscillations are difficult to unify, because they make conflicting demands on the details of how neurons interact with the oscillation. If a cell is meeting the timing requirements of selective communication with varying brain areas and simultaneously meet the requirement that spikes fall on theta peaks when a rat is at the beginning of a place field and on troughs at the end of a place field? 

Is it possible that meeting these requirements simultaneously results in strict relationships between global and local phenomenon, and that we get scientific findings from this 'for free'? For example, Fries [@fries2009model] considers one one hand the early-theta-phase coherence between entorhinal cortex and the early-theta-phase firing preference of place cells representing the rat's current position; on the other hand late-theta-phase coherence between CA3 and CA1 and the late-theta-phase preferential firing of place cells that represent positions ahead of the rat; and concludes that the CA1 at early phases processes cortical information about the current state of things, and in later phases uses the modeling capabilities of CA3 to extrapolate into the future. One reason for studying theta is to determine whether models like this are likely to be useful for understanding how information is processed and passed between brain areas.

Mizuseki et. al. [@mizuseki2009theta] point out some interesting difficulties for our understanding of systems-level theta oscillations in terms of biophysical properties of neurons and pairwise connections between brain areas. They show that cells in different brain areas tend to prefer to fire at differet theta phases that bear little reseblance to the 'sequence' implied by a synaptic traversal of the circuit.  For example, spikes of CA1 neurons occur at the opposite phase from that of their peak dendritic excitation [@kamondi1998theta; @buzsaki2002theta].  This phenomenon is acceptable to the global account of theta; it allows for the opening of 'temporal windows' of processing between sequential anatomical processing stages.  But it is at odds with intuitive and formal [@huxter2003independent; @kamondi1998theta] models of the fine timescale spiking of place cells.  Place cells in dentate gyrus are generated from the summed spiking activity of their inputs, and are expected to follow the timing of their inputs closely (TODO CITE).
