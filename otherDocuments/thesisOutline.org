...
More natural support for functional programming is available in many higher-level languages, for instance python has the builtin functions map, which takes a function and a list and returns a list with the function applied to each element:
...
#+BEGIN_SRC haskell :results function
  let topLimit2 x = if x > 10 then 10 else x
  map topLimit2 [1,15,20,3,-2,2]
#+END_SRC
...
That basic motivation for types has been elaborated to an enormous degree. The nature of types is the main feature distinguishing programming languages. Languages are typically classified in terms of their type systems; they divide languages into classes like dynamically typed languages (e.g. python, javascript, lisp) and statically typed languages (e.g. c++, Java, Haskell). An orthoginal classification still revolving around types is object-oriented (e.g. python, Java) vs. Hindley-Milner (e.g. Haskell and a large number of research languages and several new 
