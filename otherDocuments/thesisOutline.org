...
An alternative foundation is to model types around logic, capturing ideas like mutual exclusion, associated data, and value-to-value relationships in the types. An example is in order:

#+BEGIN_SRC haskell
  data Maybe a = HasAn a
               | Nothing
#+END_SRC
